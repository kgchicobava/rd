		...
		if (ev.target.value === "" || re.test(ev.target.value)) {
			...
			if (ev.target.value.length >= 7) {
				if (validator.isIP(ev.target.value)) {
					this.setState({ startIP: ev.target.value });
					// this.props.rangeIp(1, ev.target.value);
				} else {
					this.props.sendError("Your start IP address is invalid. Please check and write again", "source")
				}
			}
		}
	}
			...
			if (ev.target.value.length >= 7) {
				if (validator.isIP(ev.target.value)) {
					if (ip2int(this.state.startIP) > ip2int(this.state.endIP)) {
						this.props.sendError("Starting IP address cannot be bigger than ending", "source")
					} else {
					...
					this.props.sendError("Your end IP address is invalid. Please check and write again", "source")
				}
			}
		}
	}
		...
		if (ev.target.value === "" || re.test(ev.target.value)) {
			...
			if (ev.target.value.length >= 7) {
				if (validator.isIP(ev.target.value)) {
					...
					this.props.sendError("Your IP address is invalid. Please check and write again", "source")
				}
			}
		}
	};
					...
					{this.state.sourceValue === "IPRange" ? (
						<div>
							<Form.Field>
								...
								<Input type="text" value={this.state.startIP} onChange={this.validateStartIP} placeholder="Starting IP" />
								</Form.Field>
							<Form.Field>
								<label>Ending IP Address</label>
								<Input type="text" value={this.state.endIP} onChange={this.validateEndIP} placeholder="Ending IP" />
							</Form.Field>
						</div>
					) : (
						""
					...
					{this.state.sourceValue === "CSV" ? (
						<div>
							...
							<input
								...
								id="csv"
							/>
							<label className="select-file" htmlFor="csv">Select a file..</label>
